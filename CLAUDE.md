...
```
script อื่น: `npm start`, `npm run prisma:studio`, `npm run prisma:generate`, `npm run migrate:deploy` (prod)
...
- **Trust proxy:** `app.js` เรียก `app.set("trust proxy", 1)` ตอน prod (อยู่หลัง reverse proxy ของ host → secure cookie ทำงานถูก)
- **CORS หลาย origin:** `CLIENT_ORIGIN` ใส่ได้หลายค่าคั่น comma (prod + preview URL) — `app.js` ใช้ origin function เช็คกับ `env.clientOrigins`
- **Prisma:** `postinstall: prisma generate` (gen client ตอน install) + script `migrate:deploy` (prod ใช้ `prisma migrate deploy` ไม่ใช่ `migrate dev`)
